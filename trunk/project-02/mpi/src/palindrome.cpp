...
void runPalindrome(char* word, int s, bool& palindrome, bool& prime)
{
	...
	MPI_Send(&sum, 1, MPI_INT, SIEVE_MASTER_RANK, TAG_PRIME_QUESTION, MPI_COMM_WORLD);
...
void palindromeMaster()
{
	...
	while (!finish || slavesRunning > 0)
	{
		...
		else if (status.MPI_TAG == TAG_RUN_PALINDROME && status.MPI_SOURCE >= PALINDROME_SLAVE_RANK_START && status.MPI_SOURCE <=
			PALINDROME_SLAVE_RANK_START + PALINDROME_SLAVES_COUNT)
		{
		...
		{
			MPI_send(buffer, 1, MPI_CHAR, PALINDROME_SLAVE_RANK_START + nextSlave, TAG_RUN_PALINDROME, MPI_COMM_WORLD);
			nextSlave = (nextSlave + 1) % PALINDROME_SLAVES_COUNT;
			slavesRunning++;
		}
	}
	
	for (int i = PALINDROME_SLAVE_RANK_START; i < PALINDROME_SLAVE_RANK_START + PALINDROME_SLAVES_COUNT; ++i)
	{
...
void palindromeSlave()
{
	...
	while (loop)
	{
		...
		if (status.MPI_SOURCE == PALINDROME_MASTER_RANK && status.MPI_TAG == TAG_RUN_PALINDROME)
		{
			...
			runPalindrome(buffer, status._size, palindrome, prime);
