...
void sieveMaster()
{
	...
	while (!finish)
	{
		...
		else if (status.MPI_TAG == TAG_PRIME_QUESTION)
		{
			buffer[0] = 1;
			MPI_Send(buffer, 1, MPI_CHAR, status.MPI_SOURCE, TAG_PRIME_QUESTION, MPI_COMM_WORLD);
		}
	}
...
void sieveSlave()
{
	...
	while (!finish)
	{
		...
		if (status.MPI_SOURCE == PALINDROME_MASTER_RANK && status.MPI_TAG == TAG_FINISH)
		{
			finish = true;
		}
	}
